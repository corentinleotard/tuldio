 ...
 * - AI calls find_clients when user switches client mid-flow
...
describe('document creation flow evals', () => {
  ...
  it('calls find_clients when user switches client mid-flow', async () => {
    const resolveRounds: StoredToolRounds = [[{
      toolUseId: 'tu-resolve',
      name: 'find_clients',
      input: { search: 'Martin' },
    ...
    const scenario: EvalScenario = {
      name: 'client switch triggers find_clients',
      history: [
