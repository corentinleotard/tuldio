...
export const openDocumentTool = defineTool({
  name: 'open_document',
  description:
    `Open a document to make it the active document. Use after list_documents to select a specific quote or invoice.
Sets the document in state so its lines become visible and update_document can target it.`,
  schema: z.object({
