...
export async function findInvoiceById(input: {
  ...
  const result = await query<InvoiceRow>(
    'SELECT id, team_id, created_by, client_id, quote_id, number, title, total_ht, total_ttc, status, invoice_type, source_invoice_id, situation_number, avoir_id, pdf_url, pdp_id, pdp_status, sent_at, paid_at, cancelled_at, due_date, prestation_date, created_at FROM invoices WHERE id = $1 AND team_id = $2 LIMIT 1',
