...
export async function findInvoiceById(input: {
  ...
  const result = await query<InvoiceRow>(
    'SELECT id, team_id, created_by, client_id, quote_id, number, title, total_ht, total_ttc, status, pdf_url, sent_at, paid_at, cancelled_at, due_date, prestation_date, created_at FROM invoices WHERE id = $1 AND team_id = $2 LIMIT 1',
