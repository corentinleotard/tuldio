...
export async function updateTeamSettings(input: {
  ...
  if (sets.length === 0) {
    const result = await query<TeamRow>(
      'SELECT id, name, logo_url, original_document_url, quote_last_number, quote_validity_days, invoice_last_number, invoice_payment_delay_days, terms_accepted_at, stripe_customer_id, trial_ends_at, subscription_status, created_at FROM teams WHERE id = $1 LIMIT 1',
      [input.teamId],
    );
  ...
  const result = await query<TeamRow>(
    `UPDATE teams SET ${sets.join(', ')} WHERE id = $${idx} RETURNING id, name, logo_url, original_document_url, quote_last_number, quote_validity_days, invoice_last_number, invoice_payment_delay_days, terms_accepted_at, stripe_customer_id, trial_ends_at, subscription_status, created_at`,
    params,
  );
